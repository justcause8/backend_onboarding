...
namespace backend_onboarding.Services.Onboarding
{
    public partial class OnboardingService : IOnboardingService
    {
        ...
        public async Task<bool> UpdateAnswerAsync(int answerId, UserAnswerRequest request)
        {
            ...
            // Обновляем опции: удаляем старые, записываем новые
        ...
        {
            var answer = await _onboardingContext.Answers.FindAsync(answerId);
            if (answer == null) return false;
        ...
        public async Task<List<AnswerResponse>> GetUserAnswersByTestAsync(int userId, int testId)
        {
            return await _onboardingContext.Answers
                .Where(a => a.Fk1UserId == userId && a.Fk2Question.Fk1TestId == testId)
